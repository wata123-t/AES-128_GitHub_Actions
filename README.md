...
本記事では、AES-128暗号化回路を題材に、 **「Verilog設計 × Python検証 × クラウド分析」** を組み合わせた開発環境の構築を試みています。
最大の特徴は、単なるシミュレーションで終わらせず、検証結果をクラウドへ飛ばして解析する自動パイプラインを構築した点です。
**本プロジェクトのポイント：**
**・設計：** Verilog HDLによるAES-128の実装
**・検証：** cocotb を活用したPythonベースの効率的なテスト
**・分析：** BigQuery + dbt による検証データの統計的解析
**・自動化：** GitHub Actions を活用した、検証・分析を自動化した CI パイプライン
従来のハードウェア開発に、**ソフトウェア的な検証・自動化** を取り入れた試みについて解説します。
...
|Software|Python 3.11|テストベンチ記述・期待値生成|
|Data Analysis|BigQuery/dbt|検証ログの蓄積・分析モデルの構築
|Automation|GitHub Actions|検証・分析パイプラインの自動実行|
|Infrastructure|Docker/Docker Compose|ツールチェーンのコンテナ化|
|IaC|Terraform|検証用環境のプロビジョニング|
...
| ライブラリ | 説明 |
| :--- | :--- |
| cocotb |Python で記述できる、HDL向けのコ・シミュレーション検証フレームワーク|
| pycryptodome |AES暗号アルゴリズムのリファレンス（期待値）生成に使用|
| google-cloud-bigquery |BigQuery へのデータ投入・クエリ実行用ライブラリ|
| dbt-bigquery |	データ変換ツール「dbt」を BigQuery で動作させるためのアダプター|
...
```text
.
├── .github/workflows/   # CI/CD設定（GitHub Actions：自動パイプラインの定義）
├── aes_dbt/             # データ変換・解析（dbt：BigQuery上の統計分析モデル）
├── python/              # 検証シナリオ（cocotbテストベンチ・AES期待値生成）
├── terraform/           # IaC（Terraform：GCPインフラの自動構築・破棄）
├── verilog/             # 設計対象（AES-128 暗号化回路のRTLコード）
├── Dockerfile           # 実行環境の定義（Icarus Verilog / Python 混在環境）
├── docker-compose.yaml  # ローカル実行用の環境管理
├── Makefile             # 各種検証コマンドの共通化（CIからも呼び出し）
└── README.md            # プロジェクト概要
...
**関係記事**
本記事（第3部）は、これまでの内容を自動化させるものとなります。
**・[第1部：Verilog HDLによるAES-128の設計詳細]**
図の Verilog RTL の中身、ハードウェア設計の詳細について解説しています。
**・[第2部：Python × BigQuery × dbt によるハードウェア検証・分析]**
図の Local_or_CI での検証ロジックや、Google_Cloud でのデータ分析手法を詳しく解説しています。
**・第3部（本記事）：**
これらを GitHub Actions と Terraform で統合し、「プッシュ即検証・分析」が走るパイプラインの構築を解説します。
...
**検証の各工程（設計・分析）の詳細は、以下の記事を参照してください。**
**・[第1部：Verilog HDLによるAES-128の設計詳細]**
iverilog で動かす RTL設計の解説はこちら。
**・[第2部：Python × BigQuery × dbt による検証・分析]**
cocotb のロジックや dbt による解析手法の解説はこちら。
...
`Python`(検証シナリオ)と`pycryptodome`(期待値生成モデル)を組み合わせ、ハードウェアとソフトウェアを連携させたシミュレーションを実施します。
...
ここで改めて、ローカルからCI環境へと拡張されたシステム全体図を振り返ります。

![image.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/4239579/9746d93a-adfe-4d55-8e0e-a608ee3f0dfa.png)

`.github/workflows/`に定義されたパイプラインにより、コードをプッシュするたびに以下のプロセスが自動実行されます。

**1.🏗️ 検証環境の自動構築:** TerraformによるGCPリソースの準備
**2.⚡ シミュレーション:** iverilog + cocotb によるRTL検証
**3.📊 整合性チェック:** BigQueryへのデータ転送と dbt による統計的解析

これにより、回路修正時のデグレード（先祖返り）を即座に検知し、ハードウェア品質を常に高く保てる環境を整えました。

### ●設定ファイル(.github/workflows/verify.yml)
この設定ファイルでは、GCP認証からシミュレーション、dbt実行、そして最後のリソース削除までを一貫して定義しています。

**・環境の分離:** runs-on: ubuntu-latest により、毎回クリーンな環境で検証を実行
**・認証の自動化:** google-github-actions/auth によるセキュアなGCP連携
**・コスト対策:** if: always() を使用し、テストが失敗しても必ず terraform destroy が走るように設計
...
## 4-1.認証情報（Secrets）の設定
GitHub Actions から GCP を操作するために、リポジトリに 2 つの **Secret** を登録します。

**1. メニュー選択**
`Settings`→`Secrets and variables`→`Actions`
`New repository secret`ボタンから、以下の値を登録してください。
...
# 6.まとめ
本シリーズでは、AES-128暗号化回路を題材に、Verilog設計からクラウドでのデータ分析、そしてGitHub Actionsによる自動化まで構築してきました。

本プロジェクトでの実行内容
**ハードとソフトの協調:** cocotb を活用し、Pythonのリッチなライブラリを用いた効率的な検証を実施しました。
**Simulation結果の解析:** 膨大な結果を BigQuery 蓄積し、dbt で統計的に解析する検証を実施しました。
**自動化による品質担保:** GitHub Actions と Terraform を組み合わせ、プッシュするだけで「環境構築・検証・分析・破棄」が完結するパイプラインを構築しました。

GitHub Actions のCI環境を構築する事で、修正のたび行っていた手動の作業を省ける利便性を実感できました。
